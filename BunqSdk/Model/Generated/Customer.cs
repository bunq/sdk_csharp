...
namespace Bunq.Sdk.Model.Generated
{
    ...
    public class Customer : BunqModel
    {
        ...
        /// <summary>
        /// </summary>
        public static BunqResponse<List<Customer>> List(ApiContext apiContext, int userId,
        ...
        /// <summary>
        /// </summary>
        public static BunqResponse<Customer> Get(ApiContext apiContext, int userId, int customerId,
            IDictionary<string, string> customHeaders = null)
        {
            if (customHeaders == null) customHeaders = new Dictionary<string, string>();

            var apiClient = new ApiClient(apiContext);
            var responseRaw = apiClient.Get(string.Format(ENDPOINT_URL_READ, userId, customerId),
        ...
        public static BunqResponse<int> Update(ApiContext apiContext, IDictionary<string, object> requestMap,
            int userId, int customerId, IDictionary<string, string> customHeaders = null)
        {
            if (customHeaders == null) customHeaders = new Dictionary<string, string>();

            var apiClient = new ApiClient(apiContext);
            var requestBytes = Encoding.UTF8.GetBytes(BunqJsonConvert.SerializeObject(requestMap));
            var responseRaw = apiClient.Put(string.Format(ENDPOINT_URL_UPDATE, userId, customerId), requestBytes,
                customHeaders);
