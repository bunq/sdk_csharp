...
namespace Bunq.Sdk.Model.Generated
{
    ...
    public class DraftPayment : BunqModel
    {
        ...
        public static BunqResponse<int> Update(ApiContext apiContext, IDictionary<string, object> requestMap,
            int userId, int monetaryAccountId, int draftPaymentId, IDictionary<string, string> customHeaders = null)
        {
            if (customHeaders == null) customHeaders = new Dictionary<string, string>();
            ...
            var requestBytes = Encoding.UTF8.GetBytes(BunqJsonConvert.SerializeObject(requestMap));
            var responseRaw =
                apiClient.Put(string.Format(ENDPOINT_URL_UPDATE, userId, monetaryAccountId, draftPaymentId),
                    requestBytes, customHeaders);
        ...
        /// </summary>
        public static BunqResponse<List<DraftPayment>> List(ApiContext apiContext, int userId, int monetaryAccountId,
        ...
        /// </summary>
        public static BunqResponse<DraftPayment> Get(ApiContext apiContext, int userId, int monetaryAccountId,
            int draftPaymentId, IDictionary<string, string> customHeaders = null)
        {
            if (customHeaders == null) customHeaders = new Dictionary<string, string>();

            var apiClient = new ApiClient(apiContext);
            var responseRaw = apiClient.Get(string.Format(ENDPOINT_URL_READ, userId, monetaryAccountId, draftPaymentId),
